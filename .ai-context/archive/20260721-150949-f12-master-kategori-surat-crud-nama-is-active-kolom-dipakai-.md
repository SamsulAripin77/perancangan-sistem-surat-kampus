...
- Next exact action: migration kategori_surat, model+factory, KategoriRequest, KategoriFilter, Admin/KategoriController resource+toggle+guard, view index+modal, routes+sidebar+lang, KategoriSuratTest
- Source reference: docs/delivery/BACKLOG.md#m1-t10

## Scope

- Goal: F12 Master Kategori Surat: CRUD (nama, is_active) + kolom dipakai N template + guard hapus (extensible) + toggle
- Type: implement
- Mode: balanced
- Allowed modules:
- Explicit exclusions:
...
- Result: M1-T10 F12 Master Kategori Surat selesai: buat tabel kategori_surat (ERD §6), CRUD nama unik + kolom dipakai N template + guard hapus extensible + toggle, filter status; gating admin
- Tests/checks: php artisan test --filter=KategoriSuratTest (8 passed); full suite 97 passed; pint/phpstan/build/migrate PASS via verify.py
- Final budget result:
- Remaining risk:
