...
- Next exact action: Buat SettingService+facade, KonfigurasiController, UpdateKonfigurasiRequest, UpdateSystemSettings action, view form bergrup, override config('mail.*') di AppServiceProvider, routes+sidebar+lang, test SettingTest
- Source reference: docs/delivery/BACKLOG.md#m1-t4

## Scope

- Goal: F2 Konfigurasi Sistem (Settings): SettingService + form bergrup + Media Library logo + override config('mail.*')
- Type: implement
- Mode: balanced
- Allowed modules:
- Explicit exclusions:
...
- Result: M1-T4 F2 Konfigurasi Sistem selesai: SettingService+facade, form bergrup, logo Media Library, encrypted SMTP password, override config(mail.*)
- Tests/checks: php artisan test --filter=SettingTest (7 passed); pint/phpstan/migrate PASS via verify.py
- Final budget result:
- Remaining risk:
