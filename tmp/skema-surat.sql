...
CREATE TABLE `daftar_anggota_grup` (`id_grup_kontak` int(11), `id_grup` int(11), `nama_grup` varchar(30), `id_kontak` int(11), `nama` varchar(100), `no_hp` varchar(15), `sex` varchar(9), `alamat_sekarang` varchar(200));


DROP VIEW IF EXISTS `daftar_grup`;
CREATE TABLE `daftar_grup` (`id_grup` int(11), `nama_grup` varchar(30), `jumlah_anggota` bigint(21));


DROP VIEW IF EXISTS `daftar_kontak`;
CREATE TABLE `daftar_kontak` (`id_kontak` int(11), `id_pend` int(11), `nama` varchar(100), `no_hp` varchar(15), `sex` varchar(9), `alamat_sekarang` varchar(200));
...
CREATE TABLE `dokumen_hidup` (`id` int(11), `satuan` varchar(200), `nama` varchar(200), `enabled` int(2), `tgl_upload` timestamp, `id_pend` int(11), `kategori` tinyint(3), `attr` text, `tahun` int(4), `kategori_info_publik` tinyint(4), `updated_at` timestamp, `deleted` tinyint(1), `id_syarat` int(11), `id_parent` int(11), `created_at` timestamp, `created_by` varchar(16), `updated_by` varchar(16), `dok_warga` tinyint(1));
...
CREATE TABLE `keluarga_aktif` (`id` int(10), `no_kk` varchar(160), `nik_kepala` varchar(200), `tgl_daftar` timestamp, `kelas_sosial` int(4), `tgl_cetak_kk` datetime, `alamat` varchar(200), `id_cluster` int(11));
...
CREATE TABLE `penduduk_hidup` (`id` int(11), `nama` varchar(100), `nik` decimal(16,0), `id_kk` int(11), `kk_level` tinyint(2), `id_rtm` varchar(30), `rtm_level` int(11), `sex` tinyint(4) unsigned, `tempatlahir` varchar(100), `tanggallahir` date, `agama_id` int(1), `pendidikan_kk_id` int(1), `pendidikan_sedang_id` int(1), `pekerjaan_id` int(1), `status_kawin` tinyint(4), `warganegara_id` tinyint(4), `dokumen_pasport` varchar(45), `dokumen_kitas` varchar(45), `ayah_nik` varchar(16), `ibu_nik` varchar(16), `nama_ayah` varchar(100), `nama_ibu` varchar(100), `foto` varchar(100), `golongan_darah_id` int(11), `id_cluster` int(11), `status` int(10) unsigned, `alamat_sebelumnya` varchar(200), `alamat_sekarang` varchar(200), `status_dasar` tinyint(4), `hamil` int(1), `cacat_id` int(11), `sakit_menahun_id` int(11), `akta_lahir` varchar(40), `akta_perkawinan` varchar(40), `tanggalperkawinan` date, `akta_perceraian` varchar(40), `tanggalperceraian` date, `cara_kb_id` tinyint(2), `telepon` varchar(20), `tanggal_akhir_paspor` date, `no_kk_sebelumnya` varchar(30), `ktp_el` tinyint(4), `status_rekam` tinyint(4), `waktu_lahir` varchar(5), `tempat_dilahirkan` tinyint(2), `jenis_kelahiran` tinyint(2), `kelahiran_anak_ke` tinyint(2), `penolong_kelahiran` tinyint(2), `berat_lahir` smallint(6), `panjang_lahir` varchar(10), `tag_id_card` varchar(15), `created_at` timestamp, `created_by` int(11), `updated_at` timestamp, `updated_by` int(11), `id_asuransi` tinyint(5), `no_asuransi` char(100), `email` varchar(50));
...
CREATE TABLE `tweb_desa_pamong` (
  ...
  `bagan_warna` varchar(10) DEFAULT NULL,
...
CREATE ALGORITHM=UNDEFINED DEFINER=`u1034931`@`localhost` SQL SECURITY DEFINER VIEW `daftar_anggota_grup` AS select `a`.`id_grup_kontak` AS `id_grup_kontak`,`a`.`id_grup` AS `id_grup`,`c`.`nama_grup` AS `nama_grup`,`b`.`id_kontak` AS `id_kontak`,`b`.`nama` AS `nama`,`b`.`no_hp` AS `no_hp`,`b`.`sex` AS `sex`,`b`.`alamat_sekarang` AS `alamat_sekarang` from ((`anggota_grup_kontak` `a` left join `daftar_kontak` `b` on(`a`.`id_kontak` = `b`.`id_kontak`)) left join `kontak_grup` `c` on(`a`.`id_grup` = `c`.`id_grup`));

DROP TABLE IF EXISTS `daftar_grup`;
CREATE ALGORITHM=UNDEFINED DEFINER=`u1034931`@`localhost` SQL SECURITY DEFINER VIEW `daftar_grup` AS select `a`.`id_grup` AS `id_grup`,`a`.`nama_grup` AS `nama_grup`,(select count(`anggota_grup_kontak`.`id_kontak`) from `anggota_grup_kontak` where `a`.`id_grup` = `anggota_grup_kontak`.`id_grup`) AS `jumlah_anggota` from `kontak_grup` `a`;
